...
if [ "${agent_type}" = "coordinator" ] || [ "${CLAUDE_AGENT_TYPE:-}" = "coordinator" ]; then
  ...
  if [ -n "$final_msg" ]; then
    ...
    if [ -z "$yaml_block" ]; then
      ...
      echo 'coordinator Stop without NEXUS handoff block. Emit a ```nexus or ```yaml block per .swarms/templates/handoff.yaml before stopping.' >&2
      exit 2
    fi
    if [ -f .claude/scripts/validate-handoff.sh ]; then
      if ! printf '%s' "$yaml_block" | bash .claude/scripts/validate-handoff.sh --stdin >/dev/null 2>&1; then
        echo 'coordinator Stop with INVALID NEXUS handoff. Re-emit with all required fields per .swarms/templates/handoff.yaml.' >&2
        exit 2
      fi
    fi
  fi
