...
8. **Merge confirmation — mode-dependent (e2e-audit release-deploy-2):**
   ...
   - **Auto mode** (Cloud Routine, `--bg`, overnight) → do NOT block on a human and
     do NOT run a bare `gh pr merge`: defer to policy via the wrapper —
     `bash .claude/scripts/autonomous-ship.sh <pr>`. It runs the green-gate (arms
     native `gh pr merge --auto` ONLY when every required check is green, refuses
     otherwise), applies the `auto-merge-ok` label after its own preflight, and
     leaves the live ruleset's required checks as the real gate. If checks can't go
     green, it refuses — stop at "PR open + CI red" and surface it in the morning
     report; never improvise a merge.
9. **Squash-merge.**
   - **Interactive**: `gh pr merge --auto --squash --delete-branch` (after the user's go).
   - **Auto mode**: `autonomous-ship.sh` (step 8) already armed the squash-merge; do not
     re-run merge by hand. Once GitHub reports the PR merged, run
     `bash .claude/scripts/reconcile-shipped.sh` to flip spec/initiative status → shipped
     and append the merged PR + its proof pointers to the ship-log.
...
- **Interactive: user confirms merge. Auto mode: policy confirms merge via `autonomous-ship.sh`** — the wrapper green-gates + labels + arms `gh pr merge --auto`; a bare `gh pr merge` in auto mode is forbidden. Green required checks on a LIVE ruleset (step 0) remain the real gate. Auto mode never merges outside that policy.
