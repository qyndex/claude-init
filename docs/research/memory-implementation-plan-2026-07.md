...
1. **O-1 (blocks M-01b-auth leg) — REPRODUCED in the operator's launch context 2026-07-23.** `nohup bash -c 'claude -p --bare "ok" >out 2>&1; echo INNER_EXIT=$?'` → **`INNER_EXIT=1`, stdout contains `Not logged in · Please run /login`.** (Use single-quotes so `$?` expands *inside* `bash -c` — a double-quoted `exit=$?` reports the outer shell's `nohup`-launch code `0` and is a false pass.) Confirmed: `--bare` is the trigger, non-bare succeeds, and the auth failure **is honestly signalled by exit 1** (so M-02's `&& stamp` guard suffices; grep `Not logged in` as belt-and-suspenders). **Still needs an operator decision:** which remedy makes `--bare` authenticate — `apiKeyHelper`, exported `ANTHROPIC_API_KEY` in the spawn env, or another credential path `--bare` honors. Environment-specific; that choice is the only remaining blocker for M-01b-auth.
