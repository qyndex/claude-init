...
- **INSTALLED 2026-06-12 by operator** (`APPLY=1 staged/install.sh`, 52 files). Post-install proof: `INSTALLED=1 bash verify/2026-06-12-e2e-fixes/test-e2e-fixes.sh` → 22 suites, **361 pass / 0 fail**, `validate.sh` **0 failures** (2 environment warnings). Install surfaced 3 follow-ups, all fixed same-day: 5 state-registry rows for paths the installed hooks read (adopt, allow-skip-gates, consecutive-aborts.json, ideation-pending, tasks-md.snapshot); required-check scan tightened to claim-shaped patterns only (was false-positiving on the `auto-merge-ok` label in §XI); test-actor-artifact.sh + test-doc-claims.sh made mode-aware for their pre-install RED assertions.
